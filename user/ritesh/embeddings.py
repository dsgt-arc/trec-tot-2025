...
def create_sharded_embeddings(jsonl_path,
    ...
    with open(jsonl_path, 'r', encoding='utf-8') as f:
        ...
        if batch_texts and (max_entries is None or processed_count < max_entries):
            remaining_slots = max_entries - processed_count if max_entries else len(batch_texts)
            if max_entries:
...
def save_shard(data, shard_number, model_name, output_dir):
    ...
    # Print shard info
    print(f"Saved shard {shard_number}: {len(data):,} docs | Size: {os.path.getsize(filepath)/1024/1024:.1f} MB | File: {filename}")
