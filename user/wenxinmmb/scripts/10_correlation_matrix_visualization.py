...
def create_correlation_matrices():
    ...
    # Initialize correlation matrices (5x5)
    pearson_matrix = np.eye(5)  # Identity matrix (diagonal = 1.0)
    kendall_matrix = np.eye(5)  # Identity matrix (diagonal = 1.0)
    
    # P-value matrices
    pearson_pvalue_matrix = np.zeros((5, 5))
    kendall_pvalue_matrix = np.zeros((5, 5))
    ...
    # Set 1 vs Set 4 (Original vs Wikipedia 200w)
    pearson_matrix[0, 3] = pearson_matrix[3, 0] = 0.6199
    kendall_matrix[0, 3] = kendall_matrix[3, 0] = 0.4257
    pearson_pvalue_matrix[0, 3] = pearson_pvalue_matrix[3, 0] = 0.0000
    kendall_pvalue_matrix[0, 3] = kendall_pvalue_matrix[3, 0] = 0.0000
    
    # Set 1 vs Set 5 (Original vs Random 200w)
    pearson_matrix[0, 4] = pearson_matrix[4, 0] = 0.0784
    kendall_matrix[0, 4] = kendall_matrix[4, 0] = 0.0516
    pearson_pvalue_matrix[0, 4] = pearson_pvalue_matrix[4, 0] = 0.1251
    kendall_pvalue_matrix[0, 4] = kendall_pvalue_matrix[4, 0] = 0.1313
...
def print_correlation_summary(pearson_df, kendall_df, pearson_pvalue_df, kendall_pvalue_df):
    ...
    print("4. Wikipedia 200w: First 200 words from Wikipedia pages")
    print("5. Random 200w: Random text of 200 words")
    print()
    ...
    print("-" * 40)
    print("• Both LLM-generated queries (Gemini & GPT-4o) show very high correlation")
    print("  with original queries (r > 0.93), suggesting they capture similar semantic")
    print("  content and query characteristics.")
    print()
